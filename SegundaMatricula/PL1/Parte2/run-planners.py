...
def crear_problemas():
    ...
    while (num < 100):
        num = salto * i
        os.system("python3 generate-problem.py -d 1 -r 1" + " -l " + str((int(num // 2))) + " -p " + str((int(num))) + " -c " + str((int(num))) + " -g " + str((int(num))))
        problemas.append("drone_problem_d1_r1_l" + str((int(num // 2))) + "_p" + str((int(num))) + "_c" + str((int(num))) + "_g" + str((int(num))) + "_ct2.pddl")
        i += 1
    return problemas


def ejecutar_planificador_problema(planificador, comando, problemas):
    tiempos={}; contador=0; tiempo = 0; tiempoMax = 120000
